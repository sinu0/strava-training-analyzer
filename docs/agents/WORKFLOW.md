...
- **UI**: only `@/ui` + `Box/Stack/Grid/Typography/Button`. Missing component → add to `src/ui`, export in `index.ts`, show in `DesignSystemPage`, test in `src/ui/__tests__` (rules: `docs/DESIGN_SYSTEM.md`). UI text through `t()` from `@/i18n` — add the key to `locales/pl.ts` and `locales/en.ts`.
